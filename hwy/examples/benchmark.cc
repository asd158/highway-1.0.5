...
// These templates are not found via ADL.
        ...
        public:
            ...
            TwoArray()
                    : a_(AllocateAligned<float>(NumItems() * 2)),
                      b_(a_.get() + NumItems()) {
...
// Measures durations, verifies results, prints timings.
        ...
        void RunBenchmark(const char *caption) {
            ...
            const size_t num_results = MeasureClosure(
                    [&benchmark](const FuncInput input) { return benchmark(input); },
                    inputs,
                    kNumInputs,
                    results,
                    p);
        ...
        void Intro() {
            ...
            for (size_t              i      = 0;
                 i < 16;
                 i += Lanes(d)) {
                const auto vec    = LoadU(d,
                                          in + i);  // no alignment requirement
                auto       result = Mul(vec,
                                        vec);
                result = Add(result,
                             result);  // can update if not const
                StoreU(result,
                       d,
                       out + i);
            }
        ...
        public:
            ...
            FuncOutput operator()(const size_t num_items) {
                ...
                for (size_t i = 0;
                     i < num_items;
                     i += 4 * N) {
                    const auto a0 = Load(d,
                                         pa + i + 0 * N);
                    const auto b0 = Load(d,
                                         pb + i + 0 * N);
                    sum0 = MulAdd(a0,
                                  b0,
                                  sum0);
                    const auto a1 = Load(d,
                                         pa + i + 1 * N);
                    const auto b1 = Load(d,
                                         pb + i + 1 * N);
                    sum1 = MulAdd(a1,
                                  b1,
                                  sum1);
                    const auto a2 = Load(d,
                                         pa + i + 2 * N);
                    const auto b2 = Load(d,
                                         pb + i + 2 * N);
                    sum2 = MulAdd(a2,
                                  b2,
                                  sum2);
                    const auto a3 = Load(d,
                                         pa + i + 3 * N);
                    const auto b3 = Load(d,
                                         pb + i + 3 * N);
                    sum3 = MulAdd(a3,
                                  b3,
                                  sum3);
...
// 1.0 cycles/float = bronze, 0.7 = silver, 0.4 = gold!
        struct BenchmarkDelta : public TwoArray {
            FuncOutput operator()(const size_t num_items) const {
                ...
                for (; i < num_items;
                       i += Lanes(df)) {
                    const auto a       = Load(df,
                                              &a_[i]);
                    const auto shifted = CombineShiftRightLanes<3>(df,
                                                                   a,
                                                                   prev);
                    prev = a;
                    Store(Sub(a,
